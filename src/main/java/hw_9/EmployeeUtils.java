...
public class EmployeeUtils {
    ...
    public static double minSalary(Employee[] employees) {
        ...
        return Arrays.stream(employees).map(employee -> employee.getSalary()).min((sal1, sal2) -> {
    ...
    public static double maxSalary(Employee[] employees) {
        return Arrays.stream(employees).map(employee -> employee.getSalary()).max((sal1, sal2) -> {
            return sal1.compareTo(sal2);
        }).get();
    }
